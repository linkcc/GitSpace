...
def ArticleThumbUp_views(request):
    # 点赞数: sum
    # list = ArticleThumbUp.objects.filter(likeUser.personName = '已登录用户')
    # sum = 0
    # for obj in list:
    #     sum += 1
    ...
    if request.method == 'GET':
        ...
        # author.name=request.POST.get('upname')
        # author.age=request.POST.get('upage')
        # author.email=request.POST.get('upemail')
        # author.save()
